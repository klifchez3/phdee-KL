...
for r in range(reps):
    ### Sample the data
    log_Y_b = np.zeros((log_Y_rows,1)) # Clears out the data each run
    log_X_b = np.zeros((log_Y_rows,4))
    Y_b = np.zeros((log_Y_rows,1))
    X_b = np.zeros((log_Y_rows,4))
    for n in range(log_Y_rows):
        log_Y_b[n] = log_Y[rep_dex[n,r]]
        log_X_b[n,:] = log_X[rep_dex[n,r],:]
        Y_b[n] = log_Y[rep_dex[n,r]]
        X_b[n,:] = X[rep_dex[n,r],:]
        
    log_Y_b = np.reshape(log_Y_b,-1) # To 1d array for later conformability
    Y_b = np.reshape(Y_b,-1)
    
    ### estimate beta ols
    beta_ols_list[r,:] = np.matmul(np.linalg.inv((np.matmul(log_X_b.T, log_X_b))), np.matmul(log_X_b.T, log_Y_b)).T
...
ci_ame.iloc[3] = ' ' # There is probably a better way to do this
...
ame.iloc[3] = ' ' # There is probably a better way to do this
