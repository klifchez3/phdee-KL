...
global outputpath "C:\Users\klifchez3\GaTech Dropbox\Kelly Lifchez\phdee-KL\homework4\output"
...
reg demeaned_bycatch demeaned_shrimp demeaned_salmon demeaned_treatment demeaned_firmsize i.month, vce(cluster firm)
...
estout col1 col2 using "$outputpath/did_regression_stata.tex", replace ///
cells(b(fmt(%9.3f) star) se(par fmt(%9.3f))) ///
starlevels(* .1 ** .05 *** .01) style(tex) ///
keep(treatment demeaned_treatment firmsize demeaned_firmsize shrimp demeaned_shrimp salmon demeaned_salmon) ///
